...
void state_init() {
    ...
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_init(&open_file_table[i].file_table_lock, NULL);
...
int data_block_alloc() {
    pthread_mutex_lock(&lock_blocks);
    for (int i = 0; i < DATA_BLOCKS; i++) {
        ...
        if (free_blocks[i] == FREE) {
            free_blocks[i] = TAKEN;
            pthread_mutex_unlock(&lock_blocks);
            return i;
        }
    }
    pthread_mutex_unlock(&lock_blocks);
...
int add_to_open_file_table(int inumber, size_t offset) {
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_lock(&open_file_table[i].file_table_lock);
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            pthread_mutex_unlock(&open_file_table[i].file_table_lock);
            return i;
        }
        pthread_mutex_unlock(&open_file_table[i].file_table_lock);
    }
...
//Free open file entries -> mutex tem de proteger
//problema de proteger mapa de alocacao, por cada entrada;
...
int remove_from_open_file_table(int fhandle) {
    pthread_mutex_lock(&open_file_table[fhandle].file_table_lock);
    if (!valid_file_handle(fhandle) ||
        free_open_file_entries[fhandle] != TAKEN) {
        pthread_mutex_unlock(&open_file_table[fhandle].file_table_lock);
        return -1;
    }
